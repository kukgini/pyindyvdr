...
logging.basicConfig(level=os.getenv("LOG_LEVEL", "").upper() or None, stream=sys.stderr)
...
async def get_txn(pool: Pool, seq_no: int):
    ...
    while True:
        ...
        try:
            ...
            retry = retries > 0
            LOGGER.exception(f"Error fetching transaction (seqno: {seq_no}, retry: {retry}")
            if not retry:
                raise
            retries -= 1
...
async def get_max_seq_no(pool):
    try:
        ...
        raise Exception("Error fetching maxinum sequence number") from e
...
async def main():
    ...
    await print_txn_range(genesis_path, 1, 100)
